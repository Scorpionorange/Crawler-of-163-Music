...
public class HtmlParser {
    ...
    public List<WebPage> parsePlaylists(String url) {
        ...
        Stream<WebPage> listStream = playlists.stream().map(e -> new WebPage(BASE_URL + e.attr("href"), PageType.playlist));
        return listStream.collect(Collectors.toList());
    }

    public List<WebPage> parsePlaylist(String url) {
        Document document = Jsoup.parse(HTML_FETCHER.fetch(url));
        Elements songs = document.select("ul.f-hide li a");
        Stream<WebPage> songStream = songs.stream().map(e -> new WebPage(BASE_URL + e.attr("href"), PageType.song, e.html()));
    ...
    public static <T> void main(String[] args) throws Exception {
        ...
        System.out.println("=====================");
        System.out.println(htmlParser.parseSong("http://music.163.com/song?id=135728"));
    }
